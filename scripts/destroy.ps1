...
# Run terraform destroy with auto-approve (Reduce parallelism to avoid API throttling)
if ($Environment -eq "prod" -and (Test-Path "prod.tfvars")) {
    terraform destroy -var-file="prod.tfvars" -var="project_name=$ProjectName" -var="environment=$Environment" -parallelism=5 -auto-approve
} else {
    terraform destroy -var="project_name=$ProjectName" -var="environment=$Environment" -parallelism=5 -auto-approve
}
